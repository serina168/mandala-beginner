...
txt(s, 1.35, 1.02, 9.0, 0.44,
    "財團法人中華綜合發展研究院／文創藝術研究所", 16, color=TAUPE)
txt(s, 1.35, 1.44, 9.0, 0.40,
    "心靈藝術美學中心", 16, color=TAUPE, bold=True)
eyebrow(s, 1.35, 1.88, "MANDALA  ADVANCED  INSTRUCTOR", color=ROSE_DEEP)
txt(s, 1.0, 2.24, 9.0, 1.55, "曼陀羅心靈彩繪", 66, color=INK, bold=True)
txt(s, 1.0, 3.84, 9.0, 1.1,  "一級講師課程",   42, color=ROSE_DEEP, bold=True)
txt(s, 1.35, 5.08, 9.0, 0.55,
...
cw_s = (SW - 1.4 - 0.4) / 2; ch_s = 0.88; gy_s = 0.12
for i, (n, t, d) in enumerate(skills):
    col = i // 5; row = i % 5
    x = 0.7 + col * (cw_s + 0.4); y = 1.75 + row * (ch_s + gy_s)
    rect(s, x, y, cw_s, ch_s, WHITE, line=CREAM_DEEP, lw=0.8, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    rect(s, x, y, 0.42, ch_s, CREAM_DEEP, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
    rect(s, x + 0.22, y, 0.20, ch_s, CREAM_DEEP)
    txt(s, x + 0.02, y + 0.22, 0.38, 0.44, n, 11, color=ROSE_DEEP, bold=True, align=PP_ALIGN.CENTER)
    txt(s, x + 0.54, y + 0.08, cw_s - 0.68, 0.38, t, 14, color=ROSE_DEEP, bold=True)
    txt(s, x + 0.56, y + 0.50, cw_s - 0.72, 0.32, d, 11, color=TAUPE, spacing=1.1)
rect(s, 0.7, 6.75, SW - 1.4, 0.65, CREAM_DEEP, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
txt(s, 0.9, 6.80, SW - 1.8, 0.55,
    "考核規定：上課結束當天起三個月內完成考核作品　　"
    "照片：素色背景，提供正面、側面、中央側面特寫三個角度\n"
    "證書費用：三個月內繳交 NT$3,500　　逾期繳交 NT$4,200",
    12.5, color=INK, spacing=1.35)
